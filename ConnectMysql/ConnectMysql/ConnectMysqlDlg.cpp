...
CConnectMysqlDlg::CConnectMysqlDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(CConnectMysqlDlg::IDD, pParent)
{
...
BEGIN_MESSAGE_MAP(CConnectMysqlDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON1, &CConnectMysqlDlg::OnBnClickedButton1)
END_MESSAGE_MAP()
...
void CConnectMysqlDlg::OnBnClickedButton1()
{
	// TODO: 在此添加控件通知处理程序代码
		...
		MYSQL mysql; 	
		mysql_init(&mysql); 	
		if (!mysql_real_connect(&mysql, "localhost", "root", "2502", "students", 3306, NULL, 0)) 	
		{ 		
			MessageBox(_T("数据库连接失败")); 	
		} 	
		else 	
		{ 		
			MessageBox(_T("数据库连接chenggong"));
		}
}
